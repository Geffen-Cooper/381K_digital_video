...
class VideoPairs(Dataset):
    ...
    def __init__(self, root_dir,img_frames=True,patch_size=224,overlap_ratio=0.5,transform=None,training=True,validation=False,testing=False,frame_depth=5):
        """
        Args:
            ...
            frame_depth (int): how many frames of the video to pass into the CNN at a time
    ...
    def __getitem__(self, idx):
        ...
        video_number = idx // (w*h*100)
    
        depth_coord = (idx - video_number*(w*h*100)) // (w*h)
        x_coord = (idx - video_number*(w*h*100) - depth_coord*w*h) % w
        y_coord = (idx - video_number*(w*h*100) - depth_coord*w*h) // w
        
        # print(idx,w,h,video_number,x_coord,y_coord,depth_coord)
        ...
        # only consider frame_depth frames at a time
        # if does not fit evenly to end, just pad with zero frames
        if depth_coord < self.frame_depth//2:
            compressed = np.concatenate([np.zeros((self.frame_depth//2-depth_coord,self.patch_size[1],self.patch_size[0],3)),compressed[:depth_coord+self.frame_depth//2+1]])
            ground_truth = ground_truth[depth_coord]
        elif (99-depth_coord) < self.frame_depth//2:
            compressed = np.concatenate([compressed[depth_coord-self.frame_depth//2:],np.zeros((depth_coord-99+self.frame_depth//2,self.patch_size[1],self.patch_size[0],3))])
            ground_truth = ground_truth[depth_coord]
        else:
            # get frame_depth frames for training, and the ground truth is the middle frame
            compressed = compressed[depth_coord-self.frame_depth//2:depth_coord+self.frame_depth//2+1]
            ground_truth = ground_truth[depth_coord]
        ...
        ground_truth = torch.tensor(ground_truth.transpose(2,0,1)).float().div(255.0)

        return compressed, ground_truth

    def __len__(self):
        return self.patch_coords_x.shape[0]*self.patch_coords_x.shape[1]*self.num_videos*100 # 100 is the video depth
    ...
    def visualize_sample(self,model=None):
        ...
        aft = (torch.permute(aft[0],(1,2,3,0)).numpy()*255).astype(np.uint8)
...
def load_video_pairs(batch_size,rand_seed):
    ...
    vd_val = VideoPairs(root_dir,True,(1280,720),0.0,None,training=False,validation=True,testing=False,frame_depth=5)
    vd_test = VideoPairs(root_dir,True,(1280,720),0.0,None,training=False,validation=False,testing=True,frame_depth=5)

    # create the data loaders
    train_loader = torch.utils.data.DataLoader(vd_train, batch_size=batch_size, shuffle=True,num_workers=4)
    val_loader = torch.utils.data.DataLoader(vd_val, batch_size=4,pin_memory=True,num_workers=4)
